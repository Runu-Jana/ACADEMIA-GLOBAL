...
export async function generateMetadata({
  ...
  const t = await getTranslations('paths.detail')
  const path = await getLearningPath(slug)
  if (!path) return { title: t('notFound') }

  const title = t('metaTitle', { title: path.title })
  const description = t('metaDescription', {
    subtitle: path.subtitle,
    count: path.courseCount,
  ...
  return {
    ...
    openGraph: { title, description, url: `/paths/${slug}`, type: 'website' },
  }
}
...
export default async function PathDetailPage({
  ...
  const { slug } = await params
  const [path, t, tc] = await Promise.all([
    ...
    <>
        ...
        <div className="container relative z-10 py-9 sm:py-12">
          <nav aria-label="Breadcrumb" className="mb-5">
            <ol className="flex flex-wrap items-center gap-1.5 text-[11px] font-semibold text-white/70">
              ...
              <li><Link href="/paths" className="hover:text-white">{t('badge')}</Link></li>
              <ChevronRight aria-hidden className="h-3 w-3" />
          ...
          <div className="max-w-3xl">
            <div className="flex flex-wrap items-center gap-2">
              <Badge tone="holo">{tc(`stream.${path.stream}`)}</Badge>
              {path.featured && <Badge tone="default" className="border-white/30 text-white">{t('detail.popularTrack')}</Badge>}
            </div>
      ...
      <section className="container grid gap-10 py-9 sm:py-12 lg:grid-cols-[1fr_18rem] lg:items-start">
        ...
        <div className="min-w-0">
          <h2 className="font-display text-xl font-extrabold tracking-tight">{t('detail.stepsHeading')}</h2>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="min-w-0 flex-1 pb-1">
                    <p className="mb-2 text-[11.5px] font-bold uppercase tracking-wider text-muted-foreground">
                      {t('detail.step', { n: i + 1, level: tc(`level.${s.course.level}`) })}
                    </p>
                    <Reveal>
        ...
        <aside className="space-y-5 lg:sticky lg:top-24">
          ...
          <div className="card-base bg-muted/30 p-5">
            <p className="text-[13.5px] font-bold">{t('detail.helpTitle')}</p>
            <p className="mt-1 text-[12.5px] text-muted-foreground">{t('detail.helpBody')}</p>
            <Link
              href="/counsellor"
              className={cn(buttonVariants({ variant: 'outline', size: 'sm' }), 'mt-3 w-full')}
            >
              {t('detail.talkCounsellor')}
            </Link>
            <Link
              href="/paths"
