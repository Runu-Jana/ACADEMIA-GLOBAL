      ...
      <div className="card-base holo-ring overflow-hidden">
        <div className="border-b border-border bg-amber-50/70 p-4 dark:bg-amber-500/10">
          <Badge tone="warning">
            ...
            {t('detail.directoryBadge')}
          </Badge>
          <p className="mt-2 text-[12px] leading-relaxed text-muted-foreground">
            {t.rich('detail.notAffiliated', {
        ...
        <div className="p-4">
          <h3 className="mb-2.5 text-[14px] font-bold">{t('detail.requestHelp')}</h3>
          <LeadForm
            source="directory"
            interestedCourseId={courseId}
            defaults={defaults}
            submitLabel={t('detail.requestCallback')}
          />
        </div>
      </div>
    </div>
...
export default async function CourseDetailPage({
  ...
  const t = await getTranslations('courses')
  const modeLabel = t(`mode.${course.mode}`)
  const levelLabel = t(`level.${course.level}`)
  const streamLabel = t(`stream.${course.stream}`)

  const years = course.durationYears
  const durationLabel = t('detail.durationValue', { years })
  ...
  const tabs: TabItem[] = [
    {
      ...
      content: (
        <div className="space-y-5">
          <Panel title={t('detail.panelAbout')}>
            <p className="text-pretty text-sm leading-relaxed text-muted-foreground">
              {course.about}
            </p>
            <dl className="mt-5 grid gap-3 sm:grid-cols-3">
              {[
                { label: t('detail.statModules'), value: String(course.modules.length) },
                { label: t('detail.statLessons'), value: String(lessonCount) },
                { label: t('detail.statHours'), value: minutesLabel(totalMinutes) },
              ].map((s) => (
          ...
          <Panel title={t('detail.panelHighlights')}>
          ...
          <Panel title={t('detail.panelSkills')}>
    ...
    {
      id: 'syllabus',
      label: t('detail.tab.syllabus'),
      content: (
        <div className="space-y-2.5">
          <p className="text-[13px] text-muted-foreground">
            {t('detail.syllabusSummary', {
              modules: String(course.modules.length),
              lessons: String(lessonCount),
              content: minutesLabel(totalMinutes),
            })}
          </p>
                ...
                {m.lessons.length > 0 && (
                  <ul className="border-t border-border">
                    {m.lessons.map((l) => {
                      ...
                      const typeKey = LESSON_META[l.type] ? l.type : 'VIDEO'
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="chip hidden shrink-0 sm:inline-flex">{t(`detail.lessonType.${typeKey}`)}</span>
    ...
    {
      id: 'fees',
      label: t('detail.tab.fees'),
      content: (
        <div className="space-y-5">
          <Panel title={t('detail.panelFeeStructure')}>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[340px] text-sm">
                <caption className="sr-only">{t('detail.feeCaption', { title: course.title })}</caption>
                <thead>
                  <tr className="border-b border-border text-left text-[10px] uppercase tracking-wider text-muted-foreground">
                    <th scope="col" className="py-2.5 font-bold">{t('detail.colInstalment')}</th>
                    <th scope="col" className="py-2.5 font-bold">{t('detail.colPeriod')}</th>
                    <th scope="col" className="py-2.5 text-right font-bold">{t('detail.colAmount')}</th>
                  </tr>
                </thead>
                <tbody>
                  {instalments.map((row) => (
                    <tr key={row.n} className="border-b border-border/60">
                      <th scope="row" className="py-3 text-left text-[13px] font-bold">
                        {t('detail.yearLabel', { n: row.n })}
                      </th>
                      <td className="py-3 text-[13px] text-muted-foreground">
                        {t('detail.monthsValue', { months: row.months })}
                      </td>
            ...
            <div className="mt-4 flex items-start gap-3 rounded-xl border border-primary-100 bg-primary-50/60 p-3.5 dark:border-primary-500/25 dark:bg-primary-500/10">
              ...
              <p className="text-[13px] leading-relaxed text-muted-foreground">
                {t.rich('detail.emiNote', {
                  emi: formatINR(emi),
                  months: String(months),
                  pct: course.discountPct > 0 ? `${course.discountPct}%` : '20%',
                  b: (chunks) => <strong className="font-extrabold text-foreground">{chunks}</strong>,
                })}
              </p>
            </div>
          </Panel>

          <Panel title={t('detail.panelFeeCovers')}>
            <ul className="grid gap-2.5 sm:grid-cols-2">
              {[
                t('detail.feeCover.material'),
                t('detail.feeCover.recorded'),
                course.hasLiveClass ? t('detail.feeCover.live') : t('detail.feeCover.selfPaced'),
                t('detail.feeCover.proctored'),
                t('detail.feeCover.certificate'),
                course.hasPlacement ? t('detail.feeCover.placement') : t('detail.feeCover.mentoring'),
              ].map((item) => (
    ...
    {
      id: 'eligibility',
      label: t('detail.tab.eligibility'),
      content: (
        <div className="space-y-5">
          <Panel title={t('detail.panelWhoCanApply')}>
            ...
            <dl className="mt-4 grid gap-3 sm:grid-cols-2">
              {[
                { label: t('detail.labelLevel'), value: levelLabel },
                { label: t('detail.labelMode'), value: modeLabel },
                { label: t('detail.labelExamMode'), value: course.examMode },
                { label: t('detail.labelDuration'), value: durationLabel },
              ].map((row) => (
          ...
          <Panel title={t('detail.panelDocuments')}>
            <ul className="grid gap-2.5 sm:grid-cols-2">
              {[
                t('detail.doc.class10'),
                t('detail.doc.class12'),
                t('detail.doc.graduation'),
                t('detail.doc.govtId'),
                t('detail.doc.photo'),
                t('detail.doc.category'),
              ].map((doc) => (
    ...
    {
      id: 'university',
      label: t('detail.tab.university'),
      content: (
        <Panel title={t('detail.panelAboutUniversity')}>
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
                ...
                <span className="inline-flex items-center gap-1">
                  ...
                  {t('detail.est', { year: String(course.university.estYear) })}
                </span>
              </p>
              <Stars
          ...
          <dl className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              { icon: CalendarDays, label: t('detail.statEstablished'), value: String(course.university.estYear) },
              { icon: BookOpen, label: t('detail.statPrograms'), value: `${course.university.programs}+` },
              { icon: Users, label: t('detail.statStudents'), value: formatCount(course.university.students) },
              { icon: Award, label: t('detail.statNaac'), value: course.university.naacGrade ?? '—' },
            ].map((s) => (
          ...
          >
            {t('detail.viewUniversityProfile')}
    ...
    {
      id: 'placements',
      label: t('detail.tab.placements'),
      content: (
        <div className="space-y-5">
          <Panel title={t('detail.panelRecruiters')}>
            {recruiters.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {recruiters.map((r) => (
                  <span
              ...
              <p className="text-sm text-muted-foreground">
                {t('detail.recruitersSoon')}
              </p>
            )}
          </Panel>

          <Panel title={t('detail.panelCareerSupport')}>
            <ul className="grid gap-2.5 sm:grid-cols-2">
              {[
                t('detail.career.placementCell'),
                t('detail.career.resume'),
                t('detail.career.mockInterviews'),
                t('detail.career.softSkills'),
                t('detail.career.jobFairs'),
                t('detail.career.internships'),
              ].map((item) => (
                <li
                  key={item}
    ...
    },
    {
      ...
      content: (
        <Panel title={t('detail.studentReviews', { count: course.reviews.toLocaleString('en-IN') })}>
          <div className="mb-5 flex flex-wrap items-center gap-4 rounded-xl border border-border bg-muted/40 p-4">
            ...
            <p className="text-[13px] text-muted-foreground">
              {t.rich('detail.basedOn', {
                count: course.reviews.toLocaleString('en-IN'),
          ...
          {canReview ? (
            ...
            <p className="mb-5 rounded-xl border border-dashed border-border bg-muted/40 p-4 text-[13px] text-muted-foreground">
              {t('detail.reviewEnrol')}
            </p>
          ) : (
            <p className="mb-5 rounded-xl border border-dashed border-border bg-muted/40 p-4 text-[13px] text-muted-foreground">
              {t.rich('detail.reviewSignin', {
                link: (chunks) => (
                  <Link
          ...
          {course.courseReview.length > 0 ? (
            ...
            <div className="flex flex-col items-center gap-2 py-8 text-center">
              ...
              <p className="text-sm font-semibold">{t('detail.noReviews')}</p>
              <p className="text-[13px] text-muted-foreground">
                {t('detail.beFirst')}
              </p>
            </div>
    ...
    {
      id: 'faqs',
      label: t('detail.tab.faqs'),
      content: (
        <div className="space-y-2.5">
          <Faq
            q={t('detail.faq.ugcQ', { title: course.title })}
            a={t(course.isUgcEntitled ? 'detail.faq.ugcAYes' : 'detail.faq.ugcANo', {
              university: course.university.name,
            })}
          />
          <Faq
            q={t('detail.faq.classesQ')}
            a={t('detail.faq.classesA', {
              mode: modeLabel.toLowerCase(),
              delivery: t(course.hasLiveClass ? 'detail.faq.deliveryLive' : 'detail.faq.deliverySelf'),
              modules: String(course.modules.length),
              examMode: course.examMode.toLowerCase(),
            })}
          />
          <Faq
            q={t('detail.faq.feeQ')}
            a={t('detail.faq.feeA', {
              perYear: formatINR(course.feePerYear),
              total: formatINR(totalFee),
          ...
          />
          <Faq
            q={t('detail.faq.eligibilityQ')}
            a={t('detail.faq.eligibilityA', { eligibility: course.eligibility })}
          />
          <Faq
            q={t('detail.faq.placementQ')}
            a={t(course.hasPlacement ? 'detail.faq.placementAYes' : 'detail.faq.placementANo', {
              recruiters: recruiters.slice(0, 3).join(', ') || t('detail.faq.recruitersFallback'),
            })}
          />
        </div>
      ),
    ...
    <>
      ...
      {preview && (
        <div className="border-b border-amber-300 bg-amber-100 px-4 py-2.5 text-center text-[12.5px] font-semibold text-amber-900 dark:border-amber-500/40 dark:bg-amber-500/15 dark:text-amber-200">
          {t('detail.previewBanner', { status: course.reviewStatus })}
        </div>
      )}
        ...
        <div className="container relative z-10 py-8 sm:py-11">
          <nav aria-label="Breadcrumb">
            <ol className="flex flex-wrap items-center gap-1.5 text-[11px] font-semibold text-white/70">
              ...
              <li>
                <Link href="/courses" className="transition-colors hover:text-white">{t('courses')}</Link>
              </li>
              <ChevronRight aria-hidden className="h-3 w-3" />
          ...
          <div className="mt-3.5 flex flex-wrap items-center gap-2">
            ...
            {course.discountPct > 0 && (
              <span className="rounded-full bg-accent-green px-2.5 py-0.5 text-[11px] font-bold leading-5 text-white">
                {t('detail.off', { pct: String(course.discountPct) })}
              </span>
            )}
          </div>
          ...
          <div className="mt-6 grid grid-cols-2 gap-2.5 sm:max-w-2xl sm:grid-cols-4">
            <Fact icon={Clock} label={t('detail.factDuration')} value={durationLabel} />
            <Fact icon={Monitor} label={t('detail.factMode')} value={modeLabel} />
            <Fact
              icon={ShieldCheck}
              label={t('detail.factUgc')}
              value={course.isUgcEntitled ? t('detail.ugcEntitledShort') : t('detail.ugcRecognised')}
            />
            <Fact icon={Sparkles} label={t('detail.factAiSupport')} value={t('detail.aiMentor')} />
          </div>
        </div>
      </section>
            ...
            <div className="card-base holo-ring overflow-hidden">
              <div className="border-b border-border bg-muted/40 p-5">
                ...
                <div className="mt-1.5 flex flex-wrap items-baseline gap-2">
                  ...
                  <span className="text-xs font-semibold text-muted-foreground">{t('detail.perYear')}</span>
                </div>
                ...
                {course.originalFee && course.originalFee > course.feePerYear && (
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    ...
                    {course.discountPct > 0 && (
                      <Badge tone="success">{t('detail.save', { pct: String(course.discountPct) })}</Badge>
                    )}
                  </div>
                )}
                ...
                <dl className="mt-4 space-y-1.5 text-[13px]">
                  <div className="flex items-center justify-between gap-2">
                    <dt className="text-muted-foreground">{t('detail.total', { duration: durationLabel })}</dt>
                    <dd className="font-extrabold tabular-nums">{formatINR(totalFee)}</dd>
                  </div>
                  <div className="flex items-center justify-between gap-2">
                    <dt className="text-muted-foreground">{t('detail.emiFrom')}</dt>
                    <dd className="font-extrabold tabular-nums text-accent-green">
                      {t('detail.emiPerMonth', { amount: formatINR(emi) })}
                    </dd>
                  </div>
                </dl>
              </div>
              ...
              <ul className="space-y-2 border-t border-border bg-muted/30 p-5">
                {[
                  course.isUgcEntitled && t('detail.includeUgc'),
                  course.hasLiveClass && t('detail.includeLiveRecorded'),
                  course.hasPlacement && t('detail.includePlacement'),
                  t('detail.includeEmi'),
                ]
      ...
      {related.length > 0 && (
        <section className="container pb-12 pt-2">
          <SectionTitle
            eyebrow={t('detail.keepExploring')}
            title={t('detail.moreFrom', { university: course.university.shortName || course.university.name })}
            sub={t('detail.otherProgrammes')}
            action={
              <Link
                href={`/universities/${course.university.slug}`}
