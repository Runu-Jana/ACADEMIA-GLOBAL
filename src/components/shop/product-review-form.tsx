...
/**
 * Write / edit a buyer's own review for a shop product. Only rendered for a
 * verified buyer (the server gates on a paid order and re-checks on submit).
 * The star row is a real radio group so it works with a keyboard and a screen
 * reader, not just a mouse.
 */
export function ProductReviewForm({ productId, existing }: { productId: string; existing: Existing }) {
  ...
  async function remove() {
    ...
    setError('')
    try {
      const res = await fetch(`/api/shop/reviews?productId=${encodeURIComponent(productId)}`, {
        method: 'DELETE',
      })
      if (!res.ok) {
