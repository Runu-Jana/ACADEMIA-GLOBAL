...
// Icon/animation stay in code; the copy is keyed into home.hero.* for i18n.
const journeyCards = [
  { icon: BookOpen, key: 'learn', tone: 'from-emerald-400 to-teal-500', delay: '0s' },
  { icon: RefreshCw, key: 'restart', tone: 'from-sky-400 to-blue-500', delay: '-1.5s' },
  { icon: GraduationCap, key: 'continue', tone: 'from-amber-400 to-orange-500', delay: '-3s' },
  { icon: Trophy, key: 'succeed', tone: 'from-fuchsia-400 to-violet-500', delay: '-4.5s' },
] as const

const stats = [
  { icon: Building2, to: 1000, suffix: '+', key: 'universities' },
  { icon: BookOpen, to: 100000, suffix: '+', key: 'courses' },
  { icon: Users, to: 10, suffix: ' Lakh+', key: 'students' },
  { icon: ShieldCheck, to: 100, suffix: '%', key: 'trusted' },
] as const

export async function Hero() {
  const [t, th] = await Promise.all([getTranslations('home.hero'), getTranslations('header')])
  return (
        ...
        <div>
          ...
          <Reveal delay={300}>
            <dl className="mt-10 grid grid-cols-2 gap-x-4 gap-y-5 sm:grid-cols-4">
              {stats.map(({ icon: Icon, to, suffix, key }) => (
                <div key={key} className="flex items-center gap-2.5">
                  ...
                  <div className="min-w-0">
                    ...
                    <dt className="truncate text-[11px] font-medium text-muted-foreground">{t(`stats.${key}`)}</dt>
                  </div>
                </div>
              ))}
            </dl>
            ...
            <TiltCard className="absolute left-1/2 top-1/2 z-10 w-[38%] -translate-x-1/2 -translate-y-1/2" intensity={16} scale={1.05}>
              <div className="holo-ring holo-surface grid aspect-square place-items-center rounded-3xl border border-white/50 bg-white/80 p-5 text-center shadow-lift backdrop-blur-xl dark:border-white/10 dark:bg-slate-900/70">
                <div className="tilt-layer">
                  ...
                  <p className="font-display text-[15px] font-extrabold leading-tight text-balance">
                    {t('degreeAwaits')}
                  </p>
